...
export default class AppModel {
    ...
    static async getOrders() {
        try{
            const orderResponse = await fetch('http://localhost:4322/orders'); // get запрос по-умолчанию
            const ordersBody = await orderResponse.json();
            
            if(orderResponse.status !== 200){
                return Promise.reject(ordersBody);
            }
            console.log("Body", orderResponse);
            return ordersBody;
        } catch(err){
    ...
    static async getProducts() {
        try{
            const productResponse = await fetch('http://localhost:4322/products'); // get запрос по-умолчанию
    ...
    static async updateOrder({ orderID, name, datetime = -1 } = {
        ...
        try{
            const updateOrderResponse = await fetch(
                `http://localhost:4322/orders/${orderID}`,
                {
                    method: 'PATCH',
    ...
    // static async updateOrders({reorderedOrders = []} = {
    //     reorderedOrders: []
    // }) {
    //     try{
    //         const updateOrdersResponse = await fetch(
    //             `http://localhost:4322/orders`,
    //             {
    //                 method: 'PATCH',
    //                 body: JSON.stringify({ reorderedOrders}),
    //                 headers: {
    //                     'Content-Type': 'application/json'
    //                 }
    //             }
    ...
    //         if(updateOrdersResponse.status !== 200){
    //             const updateOrdersBody = await updateOrdersResponse.json();
    //             return Promise.reject(updateOrdersBody);
    //         }
    ...
    static async deletePosition({ positionID } = {
        ...
        try{
            const deletePositionResponse = await fetch(
                `http://localhost:4322/positions/${positionID}`,
                {
                    method: 'DELETE'
                }
            ); // get запрос по-умолчанию
