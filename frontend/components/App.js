...
export default class App {
  ...
  editTasklistName = async ({ orderID, name, datetime, trueDate }) => {
    ...
    try{
      ...
      gotta_change.querySelector('.tasklist__name').innerHTML = name;
      gotta_change.querySelector('.tasklist__date').innerHTML = trueDate;
    ...
    const okHandler = async () => {
      ...
      if(desiredElement.count >= local_count){
        if(tasklistID && modalInput.value && local_count){
          const cur_order = this.#tasklists.find(item => item.tasklistID === tasklistID);
          const cur_task = cur_order.tasks.find(item => item.prodID === modalInput.value);
          if (cur_task){
            local_count += cur_task.prodCount;
            await AppModel.deletePosition({positionID: cur_task.positionID});
            
            for(var i = 0; i < cur_order.tasks.length; i++) {
              if(cur_order.tasks[i].prodID === modalInput.value) {
                  cur_order.tasks.splice(i, 1);
                  break;
              }
            }
            document.getElementById(cur_task.positionID).remove();
          }
  ...
  async init() {
    ...
    const dataElement = document.getElementById("app-date");
    let currentDate = new Date();
    this.#working_date = currentDate.toISOString().slice(0,10); 
    dataElement.innerHTML = `Актуальная дата: ${this.#working_date}`;
    ...
    this.initEditTaskModal();
      ...
      for(const tasklist of tasklists.orders){
        ...
        for( const task of tasklist.positions){
          console.log("positions:", tasklists.orders);
          tasklistObj.addNewTaskLocal({
            tasklistID: tasklist.orderID,
            prodName: task.name,
            positionID: task.positionID,
            prodCount: task.count,
            prodID: task.product_id,
          });
